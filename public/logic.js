...
try {
  _.user.data = localStorage["babanevUser"].json();
} catch (e) {}
...
_.lazyLoad.tick = function (forceLazyLoadTick) {
  ...
  for (var i = 0; i < elementsToCheck.length; i++) {
    ...
    if (
      elementCurrentlyBeingChecked.visible(
        !_.body.hasClass("ignore-packets")
      ) ||
      elementCurrentlyBeingChecked.is(".hero") ||
      elementCurrentlyBeingChecked.closest(".window:not( .active )").length > 0
    ) {
      var elementsToLoad = $(
        "[data-src='" + elementCurrentlyBeingChecked.attr("data-src") + "']"
      );
...
_.forms.register.validate = function () {
  var fieldsToCheck = [
    ...
    {
      element: _.forms.register.$.find(
        ".field.required [name='repeat-password']"
      ),
      validation: function (value) {
        return (
          value ===
          _.forms.register.$.find(".field.required [name='password']").val()
        );
      },
    },
  ];
...
_.forms.login.validate = function () {
  ...
  if (!error) {
    _.forms.login.send(
      _.forms.login.$.find(".field.required [name='code']").val()
    );
  }
};
...
_.forms.recovery.send = function () {
  ...
  if (!response.status) {
    return _.forms.login.$.find(".field.required [name='password']")
      .closest(".field")
      .class("error")
      .find("input")
...
_.audio.cache.precached_sounds = ["media/pop.mp3"];
...
_.spinner.switch = function () {
  ...
  setTimeout(function () {
    ...
    nextStateElement
      .closest(".spinner")
      .attr("data-spinner-state-projection", newState);
...
_.document
  ...
  .on("click", "button", function () {
    ...
    setTimeout(function () {
      button
        .attr("data-current-animation-timeout", buttonTimeout)
        .class("animate");
    }, 5);
  })
  ...
  .on("click", ".cookie-banner .close", function () {
    try {
      localStorage["cookieBannerClosed"] = +new Date();
    } catch (e) {}
...
_.window
  .on("load hashchange", function (e) {
    ...
    if (
      location.hash === "#regisztracio" ||
      location.hash === "#bejelentkezes"
    ) {
